...
window.renderHistory = (query = "") => {
    ...
    const allChatArray = Object.values(window.allChats).sort((a, b) => b.timestamp - a.timestamp);
    ...
    const groupsMap = {
        pinned: { title: dict.pinned_header, items: orphanedChats.filter(c => c.pinned) },
        today: { title: dict.today_header, items: orphanedChats.filter(c => !c.pinned && c.timestamp >= dayStartTs) },
        last_7_days: { title: "Last 7 days", items: orphanedChats.filter(c => !c.pinned && c.timestamp >= weekStartTs && c.timestamp < dayStartTs) },
        dust_collecting: { title: dict.dust_collecting_header, items: orphanedChats.filter(c => !c.pinned && c.timestamp < weekStartTs) }
    };

    Object.keys(groupsMap).forEach(gKey => {
        const group = groupsMap[gKey];
        if (group.items.length > 0) {
            recentsHtml += `<div class="history-section-header mb-0.5 px-3 text-[10px] font-black uppercase tracking-widest text-zinc-400/80">${group.title}</div>`;
            recentsHtml += group.items.map(c => window.renderChatItem(c, dict)).join('');
        }
    });
