...
- Result favicons are fetched from Google's s2 favicons service — a deliberate privacy tradeoff (result hostnames are visible to Google); swap for a self-hosted proxy later if that matters.
...
| Backend error | Wave collapses (same .done state as completion); message "the cosmos is quiet right now — try again" + retry button; classic results still shown below |
...
- **Stars:** ~14 absolutely-positioned 2–3px dots generated by JS with random
  ...
  Plus one ~76×50 constellation SVG (4 dots + connecting hairlines, accent
